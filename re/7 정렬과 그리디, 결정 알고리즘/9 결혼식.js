...
*/ function solution(arrivals) {
  const times = [];

  // 도착은 +1, 떠남은 -1 이벤트로 저장
  for (let [start, end] of arrivals) {
    times.push([start, "arrive"]);
    times.push([end, "leave"]);
  }

  // 시간 정렬: 시간 오름차순 정렬, 같은 시간이면 'leave'가 먼저
  ...
  for (let [_, type] of times) {
    if (type === "arrive") current++;
    else current--;
